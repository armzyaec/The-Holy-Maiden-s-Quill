...
- Adjustable translation bubble text size and background opacity
...
You can also open the app and click **Import google_credentials.json** to copy the selected JSON key into the correct folder automatically.
...
To get this JSON file from Google Cloud:
...
8. Import the downloaded `.json` file in the app.
...
5. Adjust the translation bubble text size and background opacity if needed.
6. Press `F9` and drag over the game text.
7. Read the translated overlay.
8. Press `Esc` to hide overlays.
...
- ปรับขนาดตัวอักษรและความทึบของพื้นหลังกล่องคำแปลได้
...
หรือเปิดโปรแกรมแล้วกด **Import google_credentials.json** เพื่อเลือกไฟล์ JSON ให้โปรแกรมคัดลอกไปไว้ในโฟลเดอร์ที่ถูกต้องอัตโนมัติ
...
วิธีเอาไฟล์ JSON จาก Google Cloud:

1. เข้า Google Cloud Console: <https://console.cloud.google.com/iam-admin/serviceaccounts>
2. เลือกโปรเจกต์ที่เปิด Billing แล้ว
3. เปิดใช้งาน Cloud Vision API ในโปรเจกต์นั้น
4. ไปที่ **IAM & Admin > Service Accounts**
5. สร้างหรือเลือก service account
6. เข้า **Keys > Add key > Create new key**
...
5. ปรับขนาดตัวอักษรและความทึบของพื้นหลังกล่องคำแปลตามต้องการ
6. กด `F9` แล้วลากครอบข้อความในเกม
7. อ่านคำแปลจากกล่อง Overlay
8. กด `Esc` เพื่อซ่อนกล่องคำแปล
